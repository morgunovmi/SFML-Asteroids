    ...
    void Game::initialize() {
        ...
        for (int i = 0; i < 15; i++) {
            auto a = std::make_shared<Asteroid>(
                mDre, mWindowWidth, mWindowHeight, mRockAnimation, mUrdWidth(mDre), mUrdHeight(mDre), mUrd360(mDre), 25);
            mEntities.insert(a);
        }

        mPlayer = std::make_shared<Player>(
            mWindowWidth, mWindowHeight, mPlayerAnimation, mWindowWidth / 2, mWindowHeight / 2, 0, 20);
                ...
                if (event.key.code == sf::Keyboard::Space) {
                    if (mPlayer->isAlive()) {
                        auto b = std::make_shared<Bullet>(
                            mDre, mWindowWidth, mWindowHeight, mBulletAnimation, mPlayer->x(), mPlayer->y(), mPlayer->angle(), 10);
                        mEntities.insert(b);
                    }
                }
            }
            ...
            for (auto b : mEntities) {
                if (a->name() == ASTEROID && b->name() == BULLET) {
                    if (isCollide(a, b)) {
                        ...
                        auto e = std::make_shared<Entity>(mExplosionAnimation, a->x(), a->y());
                        e->setName(EXPLOSION);
                        mEntities.insert(e);
                        ...
                        for (int i = 0; i < 2; i++) {
                            ...
                            auto e = std::make_shared<Asteroid>(
                                mDre, mWindowWidth, mWindowHeight, mSmallRockAnimation, a->x(), a->y(), mUrd360(mDre), 15);
                            mEntities.insert(e);
                        }
                ...
                if (a->name() == PLAYER && b->name() == ASTEROID) {
                    if (isCollide(a, b)) {
                        ...
                        auto e = std::make_shared<Entity>(mShipExplosionAnimation, a->x(), a->y());
                        e->setName(EXPLOSION);
                        mEntities.insert(e);
